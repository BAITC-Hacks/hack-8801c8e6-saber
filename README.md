...
Дополнительно: закрепление важных решений, сравнение плана с предложением советника, объяснение формулы Score, отмена изменений и локальное сохранение черновика. Фронтенд работает без CDN и сборки.
...
4. **Агент с ограничениями.** Закрепите нужную меру переключателем «Закрепить» — сохраняются и мера, и район. «Найти улучшение» вызывает `POST /api/optimize`; ограничения обязательны и для AI, и для hill_climb. Советник показывает Score, бюджет, критические значения, все районы и список замен **до применения**. Проверенные шаги раскрываются отдельно. «Применить предложение» меняет план; «Отменить изменение» возвращает предыдущий вариант. Если закреплены все пять решений, поиск отключён.
...
Быстрая демонстрация: «Загрузить пример» → Score **56,54**, бюджет **95/100**, критических значений **0** → закрепить школу в Нуре → «Найти улучшение» → сравнить районы → применить → отменить. Школа остаётся в Нуре. Черновик, команда и закрепления восстанавливаются после перезагрузки; история отмены хранится до перезагрузки (последние 20 изменений).

При изменении плана старый анализ и предложение сбрасываются. Ответы на устаревшие запросы не меняют экран. При ошибке превью итоговые действия заблокированы, доступна повторная загрузка.
...
**Аналитик** (`app/ai/analyst.py`) получает посчитанные движком числа: вклады мер, синергии, критические значения, показатели до/после. Pydantic проверяет непустые строки, списки строк и ограничения длины; названный слабейший район сверяется с расчётным минимумом. При сбое выполняется одна повторная попытка с уточнением формата, затем шаблонный фолбэк. Свободный текст AI не проходит полную проверку фактической точности: источником числового результата остаётся движок.
...
`best_decisions` всегда прогоняются через `validate()` и `simulate()` движка — Score в ответе не может быть выдуман AI. Сначала считается `hill_climb()` с теми же закреплениями; если он дал больше, чем агент (или ключа нет), возвращается он: `source: "baseline"`. Если найденный Score ниже исходного — возвращается исходный набор с `delta: 0`. `ai_mode: "llm"` и `source: "baseline"` совместимы: AI участвовал в поиске, но обычный алгоритм нашёл более высокий Score.
...
Поле `locked_decisions` необязательно. Чужие решения или повторы → `422 INVALID_LOCKS`. Ответ дополнен `original_result` и `best_result` для сравнения, а расчётный Result — `score_components` с вкладами среднего, слабейшего района и штрафов (до/после/дельта). Числа округляются независимо: разность двух показанных Score может отличаться от показанной дельты на 0,01. API-документация доступна по `/docs`.
...
- `tests/frontend.test.cjs` — ответы в обратном порядке, устаревший анализ и оптимизация, ошибки сети, блокировка повторных запросов, безопасный вывод названий команд и AI-текста, сохранение закреплений и снимка для отмены.
